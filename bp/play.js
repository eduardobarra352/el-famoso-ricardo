...
exports.run = async (bot, message, args, opus) => {
        ...
        let data = opus.active.get(message.guild.id) || {};
        ...
        opus.active.set(message.guild.id, data);
}
...
function finish(bot, opus, dispatcher) {
  let fetched = opus.active.get(dispatcher.guildID);
  fetched.queue.shift();
  if (fetched.queue.lenght > 0) {
    opus.active.set(dispatcher.guildID, fetched);
    play(bot, opus, fetched);
  }
  else {
    opus.active.delete(dispatcher.guildID);
